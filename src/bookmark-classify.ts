...
export type BookmarkCategory = 'tool' | 'security' | 'technique' | 'launch' | 'research' | 'opinion' | 'commerce';
...
const TOOL_PATTERNS = [/github\.com\/[\w-]+\/[\w-]+/i, /\bnpm\s+(install|i)\b/i, /\bpip\s+install\b/i, /\bcargo\s+add\b/i, /\bbrew\s+install\b/i, /\bopen[\s-]?source\b/i, /\bcli\b.*\btool\b/i, /\btool\b.*\bcli\b/i, /\brust\s+crate\b/i, /\bvscode\s+extension\b/i, /\bnpx\s+/i, /\brepo\b.*\bgithub\b/i, /\bgithub\b.*\brepo\b/i, /\bself[\s-]?hosted\b/i, /\bopen[\s-]?sourced?\b/i];

const SECURITY_PATTERNS = [/\bcve[-\s]?\d{4}/i, /\bvulnerabilit/i, /\bexploit/i, /\bmalware\b/i, /\bransomware\b/i, /\bsupply[\s-]?chain\s+attack/i, /\bsecurity\s+(flaw|bug|issue|patch|advisory|update|breach)/i, /\bbreach\b/i, /\bbackdoor\b/i, /\bzero[\s-]?day\b/i, /\bremote\s+code\s+execution\b/i, /\brce\b/i, /\bprivilege\s+escalation\b/i, /\bcompromised?\b/i];

const TECHNIQUE_PATTERNS = [/\bhow\s+(I|we|to)\b/i, /\btutorial\b/i, /\bwalkthrough\b/i, /\bstep[\s-]?by[\s-]?step\b/i, /\bbuilt\s+(with|using|this|a|an|my)\b/i, /\bhere'?s?\s+how\b/i, /\bcode\s+(pattern|example|snippet|sample)\b/i, /\barchitecture\b.*\b(of|for|behind)\b/i, /\bimplemented?\b.*\bfrom\s+scratch\b/i, /\bunder\s+the\s+hood\b/i, /\bdeep[\s-]?dive\b/i, /\btechnique\b/i, /\bpattern\b.*\b(for|in|to)\b/i];

const LAUNCH_PATTERNS = [/\bjust\s+(launched|shipped|released|dropped|published)\b/i, /\bwe('re|\s+are)\s+(launching|shipping|releasing)\b/i, /\bannouncing\b/i, /\bintroduc(ing|es?)\b/i, /\bnow\s+(available|live|in\s+beta)\b/i, /\bv\d+\.\d+/i, /\b(alpha|beta)\s+(release|launch|is\s+here)\b/i, /\bproduct\s+hunt\b/i, /🚀.*\b(launch|ship|live)\b/i, /\bcheck\s+it\s+out\b/i];

const RESEARCH_PATTERNS = [/arxiv\.org/i, /\bpaper\b.*\b(new|our|this|the)\b/i, /\b(new|our|this)\b.*\bpaper\b/i, /\bstudy\b.*\b(finds?|shows?|reveals?)\b/i, /\bfindings?\b/i, /\bpeer[\s-]?review/i, /\bpreprint\b/i, /\bresearch\b.*\b(from|by|at|shows?)\b/i, /\bpublished\s+in\b/i, /\bjournal\b/i, /\bstate[\s-]?of[\s-]?the[\s-]?art\b/i];

const OPINION_PATTERNS = [/\bthread\b.*👇/i, /\bunpopular\s+opinion\b/i, /\bhot\s+take\b/i, /\bhere'?s?\s+(why|what|my\s+take)\b/i, /\bi\s+think\b.*\b(about|that)\b/i, /\bcontroversial\b/i, /\boverrated\b/i, /\bunderrated\b/i, /\blessons?\s+(learned|from)\b/i, /\bmistakes?\s+(I|we)\b/i];

const COMMERCE_PATTERNS = [/\bamazon\.com\b/i, /\bshop\s+(here|now)\b/i, /\bbuy\s+(now|here|this)\b/i, /\bdiscount\b/i, /\bcoupon\b/i, /\baffiliate\b/i, /\bgeni\.us\b/i, /\ba\.co\//i, /\$\d+(\.\d{2})?\s*(off|USD|discount)/i];
...
const TOOL_DOMAINS = new Set(['github.com', 'gitlab.com', 'huggingface.co', 'npmjs.com', 'pypi.org', 'crates.io', 'pkg.go.dev']);

const RESEARCH_DOMAINS = new Set(['arxiv.org', 'scholar.google.com', 'semanticscholar.org', 'biorxiv.org', 'medrxiv.org', 'nature.com', 'science.org']);

const COMMERCE_DOMAINS = new Set(['amazon.com', 'www.amazon.com', 'a.co', 'store.steampowered.com', 'geni.us', 'ebay.com']);
...
export function classifyBookmark(bookmark: BookmarkRecord): ClassifyResult {
  ...
  if (matchesAny(TOOL_PATTERNS) || githubUrls.length > 0 || domains.some((d) => TOOL_DOMAINS.has(d))) categories.push('tool');
  if (matchesAny(TECHNIQUE_PATTERNS)) categories.push('technique');
  if (matchesAny(LAUNCH_PATTERNS)) categories.push('launch');
  if (matchesAny(RESEARCH_PATTERNS) || domains.some((d) => RESEARCH_DOMAINS.has(d))) categories.push('research');
  if (matchesAny(OPINION_PATTERNS)) categories.push('opinion');
  if (matchesAny(COMMERCE_PATTERNS) || domains.some((d) => COMMERCE_DOMAINS.has(d))) categories.push('commerce');
...
export function formatClassificationSummary(summary: ClassificationSummary): string {
  const lines = [`Classified ${summary.classified}/${summary.total} bookmarks (${summary.unclassified} unclassified)`, ''];
