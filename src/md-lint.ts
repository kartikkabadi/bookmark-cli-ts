...
import {mdDir, mdIndexPath, mdCategoriesDir, mdDomainsDir, mdEntitiesDir, mdStatePath} from './paths.js';
...
export async function lintMd(): Promise<LintResult> {
  ...
  if (await pathExists(mdStatePath())) {
    try {
      mdState = await readJson<MdState>(mdStatePath());
    } catch {
      /* skip */
    }
  }
  ...
  const healthScore = totalPages === 0 ? 0 : Math.max(0, Math.round(100 - (issues.length / Math.max(totalPages, 1)) * 100));
