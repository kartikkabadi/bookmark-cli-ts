...
function firefoxBaseDir(): string {
  ...
  throw new Error(`Firefox cookie extraction is currently supported on macOS and Linux only (detected: ${os}).\n` + 'Pass cookies manually:  ft sync --cookies <ct0> <auth_token>');
}
...
export function detectFirefoxProfileDir(): string {
  ...
  const resolve = (p: {path: string; isRelative: boolean}) => (p.isRelative ? join(base, p.path) : p.path);
...
function queryFirefoxCookies(dbPath: string, host: string, names: string[]): {name: string; value: string}[] {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Could not read Firefox cookies database.\n` + `Path: ${dbPath}\n` + `Error: ${e2.message}\n` + 'If Firefox is open, try closing it and retrying.');
    } finally {
      try {
        unlinkSync(tmpDb);
      } catch {}
